...
class FreeChecking(Account):
    ...
    def withdraw(self, amount):
        ...
        ret = 0
        if self.free_withdrawals < 0 and type(ret) != str:
            ret = self.withdraw_fee
        return super().withdraw(amount+ret)
...
def add_chars(w1, w2):
    ...
    if not w1:
        return w2
    elif w1[0] == w2[0]:  # words is ordered, so it's unnecessary to do 'find()'
    ...
    if index == -1:
        return w2[0]+add_chars(w1, w2[1:])
    return add_chars(w1.replace(w1[index], ''), w2[1:])
