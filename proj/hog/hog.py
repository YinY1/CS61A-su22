...
GOAL_SCORE = 1000  # The goal of Hog is to score 100 points.
...
def oink_points(player_score, opponent_score):
    ...
    if opponent_score == 0:
        return 1
    res = ((opponent_score//10) % 10)*2 - (opponent_score % 10)
    if res > 1:
        return res
    return 1
    # END PROBLEM 2
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if delta == 0:
        ...
        if delta > 0:
            ...
            last_leader = 1
            change_flag = True
    elif last_leader == 0 and delta < 0:
        last_leader = 1
        change_flag = True
    elif last_leader == 1 and delta > 0:
        last_leader = 0
        change_flag = True
    if change_flag == True:
        msg = "Player "+str(last_leader)+" takes the lead by "+str(abs(delta))
    else:
        msg = None
    return last_leader, msg
    # END PROBLEM 6
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    This strategy is combined with oink_points_strategy and pigs_on_prime_strategy
    """
    # BEGIN PROBLEM 12
    #threshold = max(8,opponent_score-score)
    threshold = 8
    oink = oink_points(score, opponent_score)
    if pigs_on_prime(score+oink,opponent_score)+oink >= threshold:
        return 0
    pro=score/ GOAL_SCORE
    if pro <0.5:
        return 6
    elif pro < 0.3:
        return 5
