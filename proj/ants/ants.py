...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not cur_place.is_hive:
            if cur_place.bees and self.lower_bound <= distance <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        ex_damage = 0 if amount < self.health else self.damage
        for bee in self.place.bees[:]:
            bee.reduce_health(amount+ex_damage)
...
class QueenAnt(ScubaThrower):
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if gamestate.deployed_queen:
            return None
        else:
            gamestate.deployed_queen = True
